...
| Overlap | Shape, not names. Objects that sit inside one another are one part |

**Overlap** is for imports where the names carry nothing — `ComponentBody.3088`, `.3089`, `.3090`.
Two objects join when their shared volume is at least `Overlap` of the smaller one's, which is why a
chip that swallows its own pins reads as one part while the same chip standing on a board that dwarfs
...
The panel shows how many parts the rule produces, how many are multi-piece, and how big the largest
one is — so a typo in the separator or a threshold that chains the board together is visible
